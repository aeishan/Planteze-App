...
public class Bills extends AppCompatActivity implements FirebaseHelper.UserFetchListener {
        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(item == null){
                    ...
                    Double emission = Information.EMISSION_FACTOR.get(item) * Double.parseDouble(price);
                    ...
                    if(activities.get(date) == null || activities.get(date).get("Other") == null || activities.get(date).get("Other").get("Bills") == null){
                        Log.d("TEST SUBMIT", "Null Hashmap");
                        activity = new HashMap<String, Double>();
                    }
                    else{
                        Log.d("TEST SUBMIT", "Retrieving Hashmap");
                        activity = (HashMap<String, Double>) activities.get(date).get("Other").get("Bills");
                    }

                    activity.put(item , Double.parseDouble(price));
                    tracker.addActivity(date, "Other", "Bills", activity);
                    Log.d("TEST SUBMIT", user.getEmail());

                    help.saveUser(user);

                    Toast.makeText(Bills.this, emission + " kg of CO2 emitted", Toast.LENGTH_SHORT).show();
                }
